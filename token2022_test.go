...
func TestClose2022Instruction(t *testing.T) {
	var (
		account     = solana.MustPublicKeyFromBase58("83mctxW8BCh6nPGjxx4jmyaEfbpcMZpLQiv7tXVSAV7a")
		destination = solana.MustPublicKeyFromBase58("nrw1b6stoyvm3QPsh78iWoJwsjM1b7KfcvxYT3LbFun")
		owner       = solana.MustPublicKeyFromBase58("nrw1b6stoyvm3QPsh78iWoJwsjM1b7KfcvxYT3LbFun")
	)

	instruction := NewClose2022Instruction(account, destination, owner, nil)
	...
	if !built.Accounts()[0].IsWritable {
		t.Errorf("Expected account to be writable")
	}

	if !built.Accounts()[1].IsWritable {
		t.Errorf("Expected destination to be writable")
	}
